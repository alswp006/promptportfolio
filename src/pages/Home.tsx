 ...
 *   PromptPortfolio -> the app's display name
 *   생성형 AI 활용 능력이 직무 역량이 된 시대, 한국 직장인이 검증된 프롬프트를 공유·판매하고 커리어 포트폴리오로 활용하는 마켓플레이스    -> the one-line description
 */
...
export default function Home() {
  ...
  return (
    <ScreenScaffold
      top={<Top title={<Top.TitleParagraph>PromptPortfolio</Top.TitleParagraph>} />}
    >
      {/* 시각 앵커: 헤드라인 + 카드 내 진입 버튼(부유 금지, display="block" 전체폭).
          데이터 앱이면 value를 <Amount typography="t1" />(핵심 숫자)로 교체하라. */}
      <SummaryHero
        label="PromptPortfolio"
        value={<Paragraph.Text typography="t2">생성형 AI 활용 능력이 직무 역량이 된 시대, 한국 직장인이 검증된 프롬프트를 공유·판매하고 커리어 포트폴리오로 활용하는 마켓플레이스</Paragraph.Text>}
